...
export const login = async (req, res,username, password) => {
    const options = {
        'method': 'POST',
        'url': URL_PAGE,
        'headers': {
        ...
        formData: {
            ...
            'ctl00$ContentPlaceHolder1$txtTenTruyCap': username.toString(),
            'ctl00$ContentPlaceHolder1$txtMatKhau': password.toString(),
    ...
    request(options, function (error, response) {
        ...
        return responeHelper(req, res, null, false)
    });
};
...
export const getGeneralInformation = async (url) => {
    ...
    parentTable.each(function () {
        let objs = {}
        const regex = /[^(\d.\/*)]/ig;
        objs['link'] = 'http://sinhvien.ute.udn.vn/' + $(this).find('.forum-details a:nth-child(1).threadTitle').attr('href').replace(/..\/..\//g, '');
...
export const getDetail = async (uri) => {
    const body = await request({
        'method': 'GET',
        'url': uri,
    })
    ...
    let obj = {}
    let image = []
    obj['title'] = $('#sidebar-section > div > div > div.col-lg-9.col-md-12.col-sm-8.col-xs-12.content-side > section > div > div.infobox > div.right-content.pull-left.col-md-10 > div > div.col-md-9 > h1 > a > b').text()
    obj['time'] = 'Đăng ngày ' + $('#sidebar-section > div > div > div.col-lg-9.col-md-12.col-sm-8.col-xs-12.content-side > section > div > div.row > div.col-md-10.col-xs-12.col-last.post-right-content > div > div.answer-date').text().replace(/[^0-9/]/g, '')
    let img = $('#sidebar-section > div > div > div.col-lg-9.col-md-12.col-sm-8.col-xs-12.content-side > section > div > div.row > div.col-md-10.col-xs-12.col-last.post-right-content > div > div.body')
    img.children().each(function () {
        image.push('http://sinhvien.ute.udn.vn/' + $(this).attr('src'))
    })
    obj['image'] = image
    return obj
}
